...
class VCODataset(Dataset):
    ...
    def __getitem__(
        ...
        phase_shift = torch.rand(1) * 2 * np.pi
        amplitude = torch.rand(1)
        f = torch.rand(1) * 2 * np.pi
        offset = 1.5 + torch.rand(1)

        time = torch.arange(self._seq_length) * self._dt
        voltage = offset + amplitude * torch.cos(f * torch.linspace(0, 4 * np.pi, self._seq_length) + phase_shift)
...
class VCODataModule(pl.LightningDataModule):
    def __init__(
        self,
        batch_size: int = 32,
