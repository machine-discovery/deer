...
from models import MLP1, TmpScaleGRU
...
def rollout(
    ...
    if method == "multiscale_deer":
        ...
        for j in range(4):  # stack 4 set of mmultiscale (multichannel) layers consecutively
            ...
            for i in range(ch):
                def model_func(carry, inputs, params):
                    return model[(ch * j) + i].apply(params, carry, inputs)[0]
                y = seq1d(model_func, y0[i], inputs, params[(ch * j) + i], yinit_guess[i])
...
def main():
    ...
    model = tuple(TmpScaleGRU(nhidden=int(nstates / nlayer), dtype=dtype, scale=10 ** (i % nlayer)) for i in range(int(nlayer * nlayer)))
    ...
    mlp = [MLP1(nstates, nstates, dtype) for _ in range(nlayer + 1)] + [MLP1(nstates, nclass, dtype)]
    mlp = tuple(mlp)
    ...
    mlp_params = [mlp[0].init(subkey4, encoder_x)] + [
        mlp[i].init(subkey4, dummy_x) for i in range(1, nlayer + 2)
    ]
