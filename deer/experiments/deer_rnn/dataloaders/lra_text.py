...
class IMDBDataModule(pl.LightningDataModule):
    ...
    def process_dataset(self):
        ...
        max_length = self.max_length - \
            int(self.append_bos) - int(self.append_eos)
    ...
    def setup(self, stage=None):
        ...
        def collate_batch(batch):
            xs, ys = zip(*[(data["input_ids"], data["label"])
                         for data in batch])
            xs = nn.utils.rnn.pad_sequence(
                xs, padding_value=self.vocab["<pad>"], batch_first=True
            )  # (nbatch, nseq)
            # added a new dimension to make it (nbatch, nseq, ndim)
            xs = xs[..., None].type(torch.float32)
