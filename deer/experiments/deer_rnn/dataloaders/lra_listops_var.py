...
def collate_batch_pad(batch, pad_value=0):
    xs, ys = batch["sequence"], batch["label"]
    xs = torch.nn.utils.rnn.pad_sequence(
        xs, padding_value=pad_value, batch_first=True
    )  # (nbatch, nseq)
    xs = xs[..., None].type(torch.float32)  # (nbatch, nseq, ndim)
    return xs, ys
...
def listops_tokenizer(s):
    ...
    return s.translate({ord("]"): ord("X"), ord(
        "("): None, ord(")"): None}).split()


class ListOpsVarDataModule(pl.LightningDataModule):
    def __init__(
        self,
        data_dir: str,
        batch_size: int = 32,
    ...
    def process_dataset(self):
        ...
        def tokenize(x): return {"tokens": tokenizer(x["Source"])}
        dataset = dataset.map(
            tokenize,
        ...
        # Calculate lengths
        def lengths_calc(x): return {"len": len(x["tokens"])}
        dataset = dataset.map(
            lengths_calc,
            keep_in_memory=True,
            load_from_cache_file=False
        ...
        # Map vocab values
        def numericalize(x): return {"sequence": vocab(x["tokens"])}
        dataset = dataset.map(
            numericalize,
