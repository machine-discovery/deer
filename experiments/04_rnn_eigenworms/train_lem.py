...
def main():
    ...
    print(f"learning rate is {args.lr}, batch size is {batch_size}, dt is {dt}")
    ...
    for epoch in tqdm(range(args.nepochs), file=sys.stderr):
        ...
        if epoch % 1 == 0:
            ...
            for f in glob(f"{path}/last_epoch_model.pkl"):
                os.remove(f)
            checkpoint_path = os.path.join(path, "last_epoch_model.pkl")
            eqx.tree_serialise_leaves(checkpoint_path, eqx.combine(inference_params, inference_static))

            checkpoint_path = os.path.join(path, f"step_{step}.pkl")
            eqx.tree_serialise_leaves(checkpoint_path, eqx.combine(inference_params, inference_static))
            ...
            if patience_metric == "accuracy":
                if val_acc > best_val_acc:
                    ...
                    best_model = eqx.combine(inference_params, inference_static)
            ...
            summary_writer.add_scalar("test_accuracy", test_acc, step)
