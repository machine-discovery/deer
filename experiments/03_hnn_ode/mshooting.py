...
def odeint_mshooting(
    ...
    print(f"args of odeint_mshooting: {shape((f, x, t_span, params, B0, fine_steps, maxiter), print=False)}")
    solver = MSZero()
    # first-guess B0 of shooting parameters
    if B0 is None:
        _, B0 = fixed_odeint(f, x, t_span, solver.coarse_method, params)
...
def fixed_odeint(f, x, t_span, solver, params):
    ...
    while steps <= len(t_span) - 1:
        ...
        _, x, _ = solver.step(f, x, t, dt, params=params)
        sol.append(x)
        t = t + dt
...
class MSZero(nn.Module):
    ...
    def root_solve(self, f, x, t_span, B, fine_steps, maxiter, params):
        ...
        sub_t_span = np.linspace(0, dt, fine_steps)
        i = 0
        while i <= maxiter:
            i += 1
            B_coarse = fixed_odeint(f, B[i-1:], sub_t_span, solver=self.coarse_method, params=params)[1][-1]
            B_fine = fixed_odeint(f, B[i-1:], sub_t_span, solver=self.fine_method, params=params)[1][-1]
            B_out = np.zeros_like(B)
            B_out.at[:i].set(B[:i])
            B_in = B[i-1]
            for m in range(i, n_subinterv):
                B_in = fixed_odeint(f, B_in, sub_t_span, solver=self.coarse_method, params=params)[1][-1]
...
class SolverTemplate(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.order = order
        self.min_factor = np.array([min_factor])
        self.max_factor = np.array([max_factor])
        self.safety = np.array([safety])
        self.tableau = None
...
class RungeKutta4(SolverTemplate):
    def __init__(self, dtype=np.float32):
        ...
        self.dtype = dtype
        self.stepping_class = "fixed"
        self.tableau = self.construct_rk4(self.dtype)

    def construct_rk4(self, dtype):
        c = np.array([0., 1 / 2, 1 / 2, 1], dtype=dtype)
        a = [
            np.array([1 / 2], dtype=dtype),
            np.array([0., 1 / 2], dtype=dtype),
            np.array([0., 0., 1], dtype=dtype)]
        bsol = np.array([1 / 6, 1 / 3, 1 / 3, 1 / 6], dtype=dtype)
        berr = np.array([0.])
